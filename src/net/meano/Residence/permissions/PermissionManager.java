...
//			return;
		 ...
		 * (p != null) { perms = new BPermissionsAdapter();
		 * Logger.getLogger("Minecraft").log(Level.INFO,
		 * "[Residence] Found bPermissions Plugin!"); return; } p =
		 * server.getPluginManager().getPlugin("Permissions"); if (p !=
		 * null) { if
		 * (Residence.getConfigManager().useLegacyPermissions()) { perms
		 * = new LegacyPermissions(((Permissions) p).getHandler());
		 * Logger.getLogger("Minecraft").log(Level.INFO,
		 * "[Residence] Found Permissions Plugin!");
		 * Logger.getLogger("Minecraft").log(Level.INFO,
		 * "[Residence] Permissions running in Legacy mode!"); } else {
		 * perms = new OriginalPermissions(((Permissions)
		 * p).getHandler());
		 * Logger.getLogger("Minecraft").log(Level.INFO,
		 * "[Residence] Found Permissions Plugin!"); } return; }
		 */

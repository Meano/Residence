...
public class ResidenceEntityListener implements Listener {
	...
	public void onEntityDamageByEntityEvent(EntityDamageByEntityEvent event) {
		...
		if (Residence.isResAdminOn(DamagePlayer)){
			return;
		}
		FlagPermissions perms = Residence.getPermsByLocForPlayer(entity.getLocation(), DamagePlayer);
		String world = DamagePlayer.getWorld().getName();
		if((entity instanceof Villager)||(entity instanceof Animals)){
			if (!perms.playerHas(DamagePlayer.getName(), world, "destory", perms.playerHas(DamagePlayer.getName(), world, "build", true))) {
				event.setCancelled(true);
				DamagePlayer.sendMessage(ChatColor.RED + "不要试图伤害领地内的动物和村民，" + Residence.getLanguage().getPhrase("FlagDeny", "destory"));
			}
		}else if (entity instanceof Hanging){
			if (!perms.playerHas(DamagePlayer.getName(), world, "destory", perms.playerHas(DamagePlayer.getName(), world, "build", true))) {
				event.setCancelled(true);
				DamagePlayer.sendMessage(ChatColor.RED + "不要试图破坏领地内的画或物品展示框，" + Residence.getLanguage().getPhrase("FlagDeny", "destory"));
			}
		}else if(entity instanceof ArmorStand){
			if (!perms.playerHas(DamagePlayer.getName(), world, "destory", perms.playerHas(DamagePlayer.getName(), world, "build", true))) {
				event.setCancelled(true);
				DamagePlayer.sendMessage(ChatColor.RED + "不要试图破坏领地内的盔甲架，" + Residence.getLanguage().getPhrase("FlagDeny", "destory"));
			}
		}else if(entity instanceof Minecart){
			if (!perms.playerHas(DamagePlayer.getName(), world, "destory", perms.playerHas(DamagePlayer.getName(), world, "build", true))) {
				event.setCancelled(true);
				DamagePlayer.sendMessage(ChatColor.RED + "不要试图破坏领地内的矿车，" + Residence.getLanguage().getPhrase("FlagDeny", "destory"));
			}
		}
	}
